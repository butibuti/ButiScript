...
public:
	...
	BUTISCRIPT_VM_API void RestoreValue(ButiEngine::Value_ptr<void>& arg_ref_value)const override;
...
BUTISCRIPT_VM_API void PushCreateMemberInstance(CreateMemberInstanceFunction arg_function);
...
public:
	...
	//ユーザー定義型として初期化
	BUTISCRIPT_VM_API Value(ScriptClassInfo& arg_info, ButiEngine::List<ButiScript::ScriptClassInfo>* arg_p_list_scriptClassInfo);
	...
	BUTISCRIPT_VM_API static std::int32_t SetTypeIndex(std::int64_t arg_typeFunc);
	BUTISCRIPT_VM_API static std::int32_t GetTypeIndex(std::int64_t arg_typeFunc);
